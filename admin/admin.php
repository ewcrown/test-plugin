...
/**
 * Jurius Digital Admin class
 *
 * @package Jurius Digital
 * @subpackage Jurius Digital Admin
 */
class JRDIGI_Admin extends WPLNST_Core_Module {
	...
	protected function on_construct($args = null) {
		...
		// Dependencies
		jrdigi_require('core', 'text');
		
		// Load translations
		add_action('plugins_loaded', array('JRDIGI_Core_Plugin', 'load_plugin_textdomain'));
	...
	public function admin_enqueue() {
		...
		wp_enqueue_style('wplnst-admin-css', plugins_url('assets/css/admin.css', JRDIGI_FILE), array(), $this->script_version);
		
		// jQuery Lightboxed plugin
		wp_enqueue_script('wplnst-admin-lighboxed', plugins_url('assets/js/lightboxed/jquery.lightboxed.min.js', JRDIGI_FILE), array('jquery'), $this->script_version, true);
		
		// Common admin script
		wp_enqueue_script('wplnst-admin-script', plugins_url('assets/js/admin.js', JRDIGI_FILE), array('jquery'), $this->script_version, true);
		
		// Edit scan script
		if (JRDIGI_Core_Plugin::slug.'-new-scan' == $_GET['page'] || (!empty($_GET['context']) && 'edit' == $_GET['context']))
			wp_enqueue_script('wplnst-admin-script-edit', plugins_url('assets/js/admin-edit.js', JRDIGI_FILE), array('jquery', 'json2'), $this->script_version, true);
	...
	public function admin_menu() {
		...
		// Main menu
		add_menu_page($this->get_plugin_title(), $this->get_menu_title(), JRDIGI_Core_Plugin::capability, JRDIGI_Core_Plugin::slug, array(&$this, 'admin_menu_scans'), $icon_svg, '99.20226');
		
		// Scans
		add_submenu_page(JRDIGI_Core_Plugin::slug, self::get_text('scans'), self::get_text('scans'), JRDIGI_Core_Plugin::capability, JRDIGI_Core_Plugin::slug, array(&$this, 'admin_menu_scans'));
		add_submenu_page(JRDIGI_Core_Plugin::slug, self::get_text('scan_new'), self::get_text('scan_new'), JRDIGI_Core_Plugin::capability, JRDIGI_Core_Plugin::slug.'-new-scan', array(&$this, 'admin_menu_scans_new'));
		
		// Utilities
		$this->admin_menu_utilities();
		
		// Settings
		add_submenu_page(JRDIGI_Core_Plugin::slug, self::get_text('settings'), self::get_text('settings'), JRDIGI_Core_Plugin::capability, JRDIGI_Core_Plugin::slug.'-settings', array(&$this, 'admin_menu_settings'));
	...
	protected function admin_menu_addons() {
		...
		add_submenu_page(JRDIGI_Core_Plugin::slug, __('Extensions', 'jrdigi'), '<span style="color:#f18500">'.__('Extensions', 'jrdigi').'</span>', JRDIGI_Core_Plugin::capability, JRDIGI_Core_Plugin::slug.'-extensions', array(&$this, 'admin_menu_extensions'));
	}
	...
	public function admin_menu_scans() {
		jrdigi_require('admin', 'scans');
		new JRDIGI_Admin_Scans($this, 'context');
	}
	...
	public function admin_menu_scans_new() {
		jrdigi_require('admin', 'scans');
		new JRDIGI_Admin_Scans($this, 'edit');
	}
	...
	public function admin_menu_settings() {
		jrdigi_require('admin', 'settings');
		new JRDIGI_Admin_Settings($this);
	}
	...
	public function admin_menu_extensions() {
		jrdigi_require('admin', 'extensions');
		new JRDIGI_Admin_Extensions($this);
	}
	...
	private function scans_edit_submit_check() {
		...
		if ((JRDIGI_Core_Plugin::slug == $_GET['page'] && !empty($_GET['context']) && 'edit' == $_GET['context']) || JRDIGI_Core_Plugin::slug.'-new-scan' == $_GET['page']) {
	...
	public function scans_edit_submit() {
		...
		jrdigi_require('admin', 'scans-submit');
		$this->scan_submit = new JRDIGI_Admin_Scans_Submit($this->scans);
	}
	...
	private function options_screen_add() {
		...
		// Check scans context
		if (JRDIGI_Core_Plugin::slug == $_GET['page']) {
			
			// Scans list
			if (!isset($_GET['scan_id']) && empty($_GET['context'])) {
				$option_default = JRDIGI_Core_Types::scans_per_page;
				$option_per_page = 'wplnst_scans_per_page';
				$option_label = __('Scans per page', 'jrdigi');
			
			// Results list
			} elseif (isset($_GET['context']) && 'results' == $_GET['context']) {
				$option_default = JRDIGI_Core_Types::scans_results_per_page;
				$option_per_page = 'wplnst_scan_results_per_page';
				$option_label = __('Crawler results per page', 'jrdigi');
			}
		}
	...
	private static function screen_view_output($args) {
		...
		?><div class="wrap wplnst-wrap">
			...
			<?php if (!wplnst_is_curl_enabled()) : ?><div class="error notice"><p><?php _e('Not detected the required cURL module enabled. Please contact with your hosting provider in order to install cURL for PHP in this server.', 'jrdigi'); ?></p></div><?php endif; ?>
	...
	protected function get_script_version() {
		global $wp_version;
		return (empty($wp_version)? '' : 'wp-'.str_replace('http://', '', esc_url($wp_version)).'-').'wplnst-'.JRDIGI_VERSION;
	}
	...
	protected function get_menu_title() {
		return JRDIGI_Core_Plugin::title;
	}
	...
	private static function is_plugin_page() {
		global $pagenow;
		return !(empty($pagenow) || 'admin.php' != $pagenow || empty($_GET['page']) || 0 !== strpos($_GET['page'], JRDIGI_Core_Plugin::slug));
	}
